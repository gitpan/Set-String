...
Set::String - (memory efficient) unordered set of strings

=head1 SYNOPSIS

	use Set::String;

	my $set = Set::String->new( qw/foo bar baz/ );
...
Set::String implements an unordered set of strings. Set::String currently
...
=head2 new()
	...
	my $set = Set::String->new();

Creates a new Set::String object. An optionally passed hash reference can
contain options. Currently no options are supported:

	my $set = Set::String->new( { myoption => 1, foobar => 2 });

Note that:

	my $set = Set::String->new( qw/for bar baz/);
